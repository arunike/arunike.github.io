function game() {
  ...
  function getKeyAndMove() {
    switch (
      event.keyCode // Get key code
    ) {
      case 37: // Left arrow
        if (dx > -450) {
          // If character is not at the left edge of the canvas
          if (dx <= 250 && dy != 250 && dy == 0) {
            // If character is in the middle of the canvas
            dx = dx - 10;
            dir = 0;
            lArrow = "#808080";
          } else {
            // If character is at the edge of the canvas
            if (dx > 250 && dy == -250) {
              // If character is at the top right corner of the canvas
              dx = dx - 10;
              dir = 0;
              lArrow = "#808080";
            }
      ...
      case 39: // Right arrow
        if (dx <= 480) {
          // If character is not at the right edge of the canvas
          if (dx < 250 && dy != -250) {
            // If character is in the middle of the canvas
            dx = dx + 10;
            dir = 1;
      ...
      case 38: // Up arrow
        uArrow = "#808080";
        if (dx == 250 && dy >= -240) {
          // If character is at the top of the canvas
          dy = dy - 10;
          dir = 1;
        }
        break;
      ...
      case 40: // Down arrow
        dArrow = "#808080";
        if (dx == 250 && dy < 0) {
          // If character is at the bottom of the canvas
          dy = dy + 10;
          dir = 1;
        }
        break;
  ...
  function draw() {
    if (dx == 450) {
      // If character is at the right edge of the canvas
      sky = "#002233";
      tod = 1;
    }
    if (dx == -450) {
      // If character is at the left edge of the canvas
      sky = "#cceeff";
      tod = 0;
    }
    ...
    if (tod == 0) {
      ...
      mat3.scale(stack[0], stack[0], [0.5, 0.5]);
      clouds();
    ...
    stack.unshift(mat3.clone(stack[0])); // Save current transformation
    mat3.scale(stack[0], stack[0], [0.3, 0.3]); // Scale
...
function start() {
  ...
  var canvas = document.getElementById("myCanvas");
  canvas.style =
    "position： top: 0px; left: 0px; right: 0px; bottom: 0px; margin: auto; border: 5px solid black;";
