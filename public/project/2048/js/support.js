...
function getNumberBackgroundColor(number) {
  switch (
    number // Switch statement to return the background color
  ) {
    case 2:
      return "#eee4da";
      break;
...
function getNumberColor(number) {
  if (number <= 4) {
    // If the number is less than or equal to 4, return white
    return "#776e65";
  }
    ...
    for (var j = 1; j < 4; j++) {
      ...
      if (board[i][j] != 0) {
        ...
        if (board[i][j - 1] == 0 || board[i][j - 1] == board[i][j]) {
          // If the board is empty or the number is the same
          return true;
        }
      }
    }
